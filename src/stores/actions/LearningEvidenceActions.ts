...
import { MUTATION_CHANGE_ACTIVE_LEARNING_EVIDENCE, MUTATION_CREATE_LEARNING_EVIDENCE, MUTATION_DELETE_LEARNING_EVIDENCE, MUTATION_UPDATE_LEARNING_EVIDENCE } from '../graphql/LearningEvidence/LearningEvidenceMutations';
...
export const dataLearningEvidence = (id: any) => {
  return async (dispatch: any) => {
    try {
      let data = {};
      await client
        .query({
          query: QUERY_GET_LEARNING_EVIDENCE,
          variables: {
            id,
          },
        })
...
export const saveNewLearningEvidence = (data: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_CREATE_LEARNING_EVIDENCE,
...
export const updateLearningEvidence = (data: any, id: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_UPDATE_LEARNING_EVIDENCE,
...
export const changeActiveLearningEvidence = (active: any, id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataChangeActive = null;
      await client
        .mutate({
          mutation: MUTATION_CHANGE_ACTIVE_LEARNING_EVIDENCE,
          variables: { id, active },
        })
        .then((dataReponse: any) => {
          if (dataReponse.errors?.length > 0) {
...
export const deleteLearningEvidence = (id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataDelete = null;
      await client
        .mutate({
          mutation: MUTATION_DELETE_LEARNING_EVIDENCE,
          variables: { id },
        })
        .then((dataReponse: any) => {
          if (dataReponse.errors?.length > 0) {
