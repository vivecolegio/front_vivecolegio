...
export const dataSpeciality = (id: any) => {
  return async (dispatch: any) => {
    try {
      let data = {};
      await client
        .query({
          query: QUERY_GET_SPECIALITY,
          variables: {
            id,
          },
        })
...
export const saveNewSpeciality = (data: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_CREATE_SPECIALITY,
...
export const updateSpeciality = (data: any, id: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_UPDATE_SPECIALITY,
...
export const changeActiveSpeciality = (active: any, id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataChangeActive = null;
      await client
        .mutate({
          mutation: MUTATION_CHANGE_ACTIVE_SPECIALITY,
          variables: { id, active },
        })
        .then((dataReponse: any) => {
          if (dataReponse.errors?.length > 0) {
...
export const deleteSpeciality = (id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataDelete = null;
      await client
