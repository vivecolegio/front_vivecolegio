...
export const getListAllMunicipality = () => {
  return async (dispatch: any) => {
    try {
      let listData = {};
      await client
        .query({
          query: QUERY_GET_ALL_MUNICIPALITY,
        })
        .then((result: any) => {
...
export const dataMunicipality = (id: any) => {
  return async (dispatch: any) => {
    try {
      let data = {};
      await client
        .query({
          query: QUERY_GET_MUNICIPALITY,
          variables: {
            id,
          },
        })
...
export const saveNewMunicipality = (data: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_CREATE_MUNICIPALITY,
...
export const updateMunicipality = (data: any, id: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_UPDATE_MUNICIPALITY,
...
export const changeActiveMunicipality = (active: any, id: any) => {
  return async (dispatch: any) => {
    try {
      let dataChangeActive = null;
      await client
        .mutate({
          mutation: MUTATION_CHANGE_ACTIVE_MUNICIPALITY,
          variables: { id, active },
        })
        .then((dataReponse: any) => {
          if (dataReponse.errors?.length > 0) {
