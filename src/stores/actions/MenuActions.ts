import { MENU_CHANGE_HAS_SUB_ITEM_STATUS, MENU_CHANGE_DEFAULT_CLASSES, MENU_CONTAINER_ADD_CLASSNAME, MENU_CLICK_MOBILE_MENU, MENU_SET_CLASSNAMES } from "../reducers/types/menuTypes";
...
export const clickOnMobileMenu = (strCurrentClasses: any) => {
  const currentClasses = strCurrentClasses
    ? strCurrentClasses
        .split(' ')
        .filter((x: string) => {return x !== '' && x !== 'sub-show-temporary'})
    : '';
  let nextClasses = '';
  if (currentClasses.includes('main-show-temporary')) {
    nextClasses = currentClasses
      .filter((x: string) => {return x !== 'main-show-temporary'})
      .join(' ');
  } else {
...
export const setContainerClassnames = (
  ...
  const currentClasses = strCurrentClasses
    ? strCurrentClasses.split(' ').filter((x: string) => {return x !== ''})
    : '';
  let nextClasses = '';
