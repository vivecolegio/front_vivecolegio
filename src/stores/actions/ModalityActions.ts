...
export const getListAllModality = (schoolId:string) => {
...
export const dataModality = (id: any) => {
  return async (dispatch: any) => {
    try {
      let data = {};
      await client
        .query({
          query: QUERY_GET_MODALITY,
          variables: {
            id,
          },
        })
...
export const saveNewModality = (data: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_CREATE_MODALITY,
...
export const updateModality = (data: any, id: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_UPDATE_MODALITY,
...
export const changeActiveModality = (active: any, id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataChangeActive = null;
      await client
        .mutate({
          mutation: MUTATION_CHANGE_ACTIVE_MODALITY,
          variables: { id, active },
        })
        .then((dataReponse: any) => {
          if (dataReponse.errors?.length > 0) {
...
export const deleteModality = (id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataDelete = null;
      await client
