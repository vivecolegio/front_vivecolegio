...
export const dataCampus = (id: any) => {
  return async (dispatch: any) => {
    try {
      let data = {};
      await client
        .query({
          query: QUERY_GET_CAMPUS,
          variables: {
            id,
          },
        })
...
export const saveNewCampus = (data: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_CREATE_CAMPUS,
...
export const updateCampus = (data: any, id: any) => {
  return async (dispatch: any) => {
    try {
      let model: {};
      model = {
      ...
      await client
        .mutate({
          mutation: MUTATION_UPDATE_CAMPUS,
...
export const changeActiveCampus = (active: any, id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataChangeActive = null;
      await client
        .mutate({
          mutation: MUTATION_CHANGE_ACTIVE_CAMPUS,
          variables: { id, active },
        })
        .then((dataReponse: any) => {
          if (dataReponse.errors?.length > 0) {
...
export const deleteCampus = (id: any, showToast: boolean) => {
  return async (dispatch: any) => {
    try {
      let dataDelete = null;
      await client
