...
const GraphicsCourse = (props: any) => {
  ...
  useEffect(() => {
    getDataChart(props?.loginReducer?.schoolId, props?.data);
  }, [props?.data]);
  ...
  const getDataChart = async (schoolId: any, academicGradeId: any) => {
    props.dataGraphicsStudentAcademicCourse(schoolId, academicGradeId).then((listData: any) => {
      setDataChart(
        listData.map((c: any) => {
          return { category: c.node.name + ' - ' + c.node.campus.name, value: c.node.countStudent };
        }),
      );
    });
  };
...
export default connect(mapStateToProps, mapDispatchToProps)(GraphicsCourse);
