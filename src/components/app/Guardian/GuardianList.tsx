...
const GuardianList = (props: any) => {
  ...
  const onSubmit = async (dataForm: any) => {
    if (data === null) {
      await props.saveNewGuardian(dataForm).then((id: any) => {
        if (id !== undefined) {
          setModalOpen(false);
          refreshDataTable();
        }
      ...
      await props.updateGuardian(dataForm, data.id).then((id: any) => {
        if (id !== undefined) {
          setModalOpen(false);
          setData(null);
          refreshDataTable();
  ...
  const viewEditData = async (id: any) => {
    await props.dataGuardian(id).then((formData: any) => {
  ...
  const deleteAll = async (items: any) => {
    items.map(async (item: any) => {
      await props.deleteGuardian(item.id, false).then(
        () => { },
        () => {
  ...
  const changeActiveDataAll = async (items: any) => {
    items.map(async (item: any) => {
      await props.changeActiveGuardian(!item.active, item.id, false).then(
        () => { },
        () => {
          createNotification('error', 'error', '');
        },
...
const mapDispatchToProps = { ...guardianActions, ...userActions };
