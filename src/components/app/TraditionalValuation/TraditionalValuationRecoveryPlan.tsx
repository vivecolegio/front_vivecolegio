...
const ExperienceLearningTraditionalValuationRecoveryPlanList = (props: any) => {
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <>
          {valuations !== null && students !== null && averagesFinal !== null ? (
            <>
              <table className="table table-bordered">
                ...
                <tbody>
                  {students.map((itemStudent: any, indexE: any) => {
                    let averageFinal = averagesFinal?.filter((itemV: any) => itemV?.node?.studentId == itemStudent?.id);
                    let show = false;
                    for (let final of averageFinal) {
                      if (final?.node?.valuationType !== "RECOVERY") {
                        if (final?.node?.performanceLevel?.isRecovery) {
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(ExperienceLearningTraditionalValuationRecoveryPlanList);
