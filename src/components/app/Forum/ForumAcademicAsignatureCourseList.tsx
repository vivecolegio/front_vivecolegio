...
const ForumAcademicAsignatureCourseList = (props: any) => {
  ...
  const additionalFunction = async (item: any, btn: any) => {
    switch (btn?.action) {
      case 'goToChildrenSpredsheet':
        goToChildren(
          `/forumList?gradeId=${item?.course?.academicGradeId}&gradeName=${item?.course?.academicGrade?.name}&courseName=${item?.course?.name}&courseId=${item?.course?.id}&academicAsignatureCourseId=${item?.id}&asignatureId=${item.academicAsignatureId}&asignatureName=${item.academicAsignature?.name}`,
        );
        break;
      default:
        break;
...
export default connect(mapStateToProps, mapDispatchToProps)(ForumAcademicAsignatureCourseList);
