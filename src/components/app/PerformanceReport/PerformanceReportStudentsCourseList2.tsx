...
const PerformanceReportStudentCourseList2 = (props: any) => {
  ...
  const generatePerformanceReportCourse2 = async (format: any) => {
    setLoading(true);
    props.generatePerformanceReportCourse2(courseId, props?.loginReducer?.schoolId,
  ...
  const generatePerformanceReportCourseStudent2 = async (studentId: any, format: any) => {
    setLoading(true);
    props.generatePerformanceReportCourseStudent2(courseId, props?.loginReducer?.schoolId,
        ...
        <div className='d-flex justify-content-between align-items-center'>
          ...
          <div>
            <div className="d-flex justify-content-start align-items-center" >
              <button
                onClick={() => {
                  return generatePerformanceReportCourse2("Letter");
                  //return getSpreadsheet(item?.node?.id);
                }}
                className={`ml-1 btn btn-info`}
                type="button"
              >
                <i className="iconsminds-download"></i> {'Final - Carta'}
              </button>{'  '}
            </div>
            <div className="d-flex justify-content-start align-items-center mt-2" >
              <button
                onClick={() => {
                  return generatePerformanceReportCourse2("Legal");
                  //return getSpreadsheet(item?.node?.id);
                }}
                className={`ml-1 btn btn-info`}
                type="button"
              >
                <i className="iconsminds-download"></i>  {'Final - Oficio'}
              </button>{'  '}
            </div>
          </div>
        </div>
                    ...
                    <>
                      <tr key={index}>
                        ...
                        <td>
                          <button
                            onClick={() => {
                              return generatePerformanceReportCourseStudent2(itemStudent?.id, "Letter");
                              //return getSpreadsheet(item?.node?.id);
                            }}
                            className={`ml-1 btn btn-info`}
                            type="button"
                          ...
                          <button
                            onClick={() => {
                              return generatePerformanceReportCourseStudent2(itemStudent?.id, "Legal");
                              //return getSpreadsheet(item?.node?.id);
                            }}
                            className={`ml-1 btn btn-info`}
                            type="button"
...
export default connect(mapStateToProps, mapDispatchToProps)(PerformanceReportStudentCourseList2);
