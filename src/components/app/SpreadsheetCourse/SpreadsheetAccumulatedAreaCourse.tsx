...
const SpreadsheetAccumulatedAreaCourse = (props: any) => {
                    ...
                    promisesListAreas.push(
                      props
                        ...
                        .then(async (notesFinal: any) => {
                          if (ntsArea[area?.id] != null) {
                            ntsArea[area?.id] = [...ntsArea[area?.id], ...notesFinal.data.edges];
                          } else {
                  ...
                  <tr>
                    ...
                    {areas?.map((item: any, index: any) => {
                      return (
                        <>
                          <th
                            colSpan={(academicPeriods?.length + 1)}
                            ...
                            return (
                              <>
                                {academicPeriods.map((itemPeriod: any) => {
                                  ...
                                  let valuationArea = valuationsArea[itemArea?.id]?.filter((itemA: any) => itemA?.node?.studentId == itemStudent?.id && itemA?.node?.academicPeriodId == itemPeriod?.node?.id.toString());
...
export default connect(mapStateToProps, mapDispatchToProps)(SpreadsheetAccumulatedAreaCourse);
